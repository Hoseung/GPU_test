...
void TestScheme::testSigmoid(long logq, long logp, long logn, long degree) {
    ...
    cout << "cipher "<< cipher.logp << " " << cipher.logq << " \n";
    cout << "csig " << csig.logp << " " << csig.logq << " \n";
	timeutils.start(SIGMOID);
	algo.function(csig, cipher, SIGMOID, logp, degree);
	timeutils.stop(SIGMOID);

    cout << "csig after " << csig.logp << " " << csig.logq << " \n";
...
void TestScheme::testWriteAndRead(long logq, long logp, long logSlots) {
	...
	Ring ring;
	SecretKey secretKey(ring);
    Scheme scheme(secretKey, ring, true, "/home/hoseung/Work/fhe-ai-sw-etri/fase/HEAAN/run/here/");


    Ring ring2;
    cout << "ring2" << std::endl;
	Scheme scheme2(ring2, true, "/home/hoseung/Work/fhe-ai-sw-etri/fase/HEAAN/run/here/");
	long n = (1 << logSlots);
