...
chrono::high_resolution_clock::time_point c1, c2;
#define START() c1 = chrono::high_resolution_clock::now();
#define END() c2 = chrono::high_resolution_clock::now();
#define SUM(t) t += (double)chrono::duration_cast<chrono::nanoseconds >(c2-c1).count() ;
#define PRINTTIME(msg) std::cout << msg << " time = " << (double)chrono::duration_cast<chrono::nanoseconds >(c2-c1).count() / 1000 << " microseconds" << std::endl;
...
RingMultiplier::RingMultiplier(long logN, long logQ) : logN(logN) {
	...
	cout << bound << " " << PRIME_BIT_SIZE <<endl;
...
uint64_t* RingMultiplier::toNTT(NTL::ZZ* x, long np) {
	using namespace NTL;
...
void RingMultiplier::reconstruct(NTL::ZZ* x, uint64_t* rx, long np, NTL::ZZ& mod) {
	using namespace NTL;
	ZZ* pHatnp = pHat[np - 1];
...
void RingMultiplier::mult(NTL::ZZ* x, NTL::ZZ* a, NTL::ZZ* b, long np, NTL::ZZ& mod) {
	
	double ntt_time=0,crt_time=0,icrt_time=0,mul_time=0;
	chrono::high_resolution_clock::time_point start1, end1;

	NTL::ZZ* test = new  NTL::ZZ[N];
	uint64_t* rt = cuda_host_mult( test,  a,  b,  np, mod);
	using namespace NTL;
	...
	uint64_t* rx = new uint64_t[np << logN]();
	// uint64_t* test = new uint64_t[np << logN]();
	cout << "\n\n cpu start \n" << endl;
	...
	for (long i = first; i < last; ++i) {
		...
		for( long k =0;k<N;k++)
		{
			if(rt[k+(i << logN)] != rai[k])
			{
				//cout << rt[k+(i << logN)] << ": " <<rai[k]<< ": " << k << endl;
				//cout << NTL_ZZ_NBITS << endl;
			}
		}
	...
	cout<<" cpu CRT TIME :" << crt_time/1000<<  endl;
	cout<<" cpu NTT TIME :" << ntt_time/1000<<  endl;
	cout<<" cpu MUL TIME :" << mul_time/1000<<  endl;
	cout<<" cpu ICRT TIME :" << icrt_time/1000<<  endl;

	cout << "CPU time = " << (double)chrono::duration_cast<std::chrono::nanoseconds >(end1 - start1).count() / 1000 << " microseconds" << endl;
...
void RingMultiplier::multNTT(NTL::ZZ* x, NTL::ZZ* a, uint64_t* rb, long np, NTL::ZZ& mod) {
	uint64_t* ra = new uint64_t[np << logN]();
	uint64_t* rx = new uint64_t[np << logN]();
...
void RingMultiplier::multDNTT(NTL::ZZ* x, uint64_t* ra, uint64_t* rb, long np, NTL::ZZ& mod) {
	uint64_t* rx = new uint64_t[np << logN]();
...
void RingMultiplier::multAndEqual(NTL::ZZ* a, NTL::ZZ* b, long np, NTL::ZZ& mod) {
	uint64_t* ra = new uint64_t[np << logN]();
	uint64_t* rb = new uint64_t[np << logN]();
...
void RingMultiplier::multNTTAndEqual(NTL::ZZ* a, uint64_t* rb, long np, NTL::ZZ& mod) {
	uint64_t* ra = new uint64_t[np << logN]();
...
void RingMultiplier::square(NTL::ZZ* x, NTL::ZZ* a, long np, NTL::ZZ& mod) {
	uint64_t* ra = new uint64_t[np << logN]();
	uint64_t* rx = new uint64_t[np << logN]();
...
void RingMultiplier::squareNTT(NTL::ZZ* x, uint64_t* ra, long np, NTL::ZZ& mod) {
	uint64_t* rx = new uint64_t[np << logN]();
...
void RingMultiplier::squareAndEqual(NTL::ZZ* a, long np, NTL::ZZ& mod) {
	uint64_t* ra = new uint64_t[np << logN]();
